...
if (isset($_REQUEST["my_message"]) && empty($_REQUEST["robotblock"])) {
	...
	// If the user has not attached a file to the this message
	if ($_SESSION['file_id'] == 'no_file_attached') {
		...
		// If the user has attached a new file to the message
		
		$data = [
		    'role' => 'user',
		    'content' => $message,
			'file_ids' => [$_SESSION['file_id']]
		];
	...
	if ($httpCode == 200) {
		...
		// Clear the file_id session variable
		$_SESSION['file_id'] = 'no_file_attached';
